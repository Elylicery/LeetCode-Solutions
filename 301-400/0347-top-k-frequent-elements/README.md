...
时间复杂度分析：O(n + m log m)（前置遍历数组 O(n) + 排序 O(m log m)

空间复杂度：O(m)（m 是 nums 中不同元素的个数，m ≤ n，Map 存储 m 个键值对）
...
时间复杂度：O (n + k log k + (m - k) log k) = O (n + m log k)，k 越小时优势越明显。
...
function topKFrequentMinHeap(nums: number[], k: number): number[] {
  ...
  class MyMinHeap<E extends number> {
    ...
    private siftUp(k: number): void {
      ...
      while (k > 0 && frequencyMap.get(this.data[k])! < frequencyMap.get(this.data[this.parent(k)])!) {
